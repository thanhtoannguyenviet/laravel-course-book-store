...
<div class="container mx-auto p-4">
    <h1 class="text-2xl font-bold mb-4">Thêm sách mới</h1>
    ...
    <form action="{{ route('admin.books.store') }}" method="post">
        ...
        <div class="mb-2">
            ...
            <input type="text" name="title" value="{{ old('title') }}" class="border p-1 w-full" required>
        ...
        <div class="mb-2">
            ...
            <input type="url" name="cover_url" value="{{ old('cover_url') }}" class="border p-1 w-full">
            @error('cover_url')<p class="text-red-600">{{ $message }}</p>@enderror
        </div>

        <div class="mb-2">
            <label class="block">Original Price *</label>
            <input type="number" step="0.01" name="original_price" value="{{ old('original_price') }}" class="border p-1 w-full" required>
            @error('original_price')<p class="text-red-600">{{ $message }}</p>@enderror
        </div>

        <div class="mb-2">
            <label class="block">Selling Price *</label>
            <input type="number" step="0.01" name="selling_price" value="{{ old('selling_price') }}" class="border p-1 w-full" required>
            @error('selling_price')<p class="text-red-600">{{ $message }}</p>@enderror
        </div>

        <div class="mb-2">
            <label class="block">Stock Quantity *</label>
            <input type="number" name="stock_quantity" value="{{ old('stock_quantity') }}" class="border p-1 w-full" required>
            @error('stock_quantity')<p class="text-red-600">{{ $message }}</p>@enderror
        </div>

        <div class="mb-2">
            <label class="block">Sold Quantity *</label>
            <input type="number" name="sold_quantity" value="{{ old('sold_quantity') }}" class="border p-1 w-full" required>
